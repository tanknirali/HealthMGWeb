        ...
        $.ajax({
			...
			success: function (othersData){
				...
				for (i = 0; i < result.length; i++)
				{
						...
						$othersData.append('<li>'+result[i]['permissionBy']+'</li><button class="btn btn-primary" id="other-cal-button" title="'+result[i]['permissionBy']+'"'+calbutton+'>Calories</button> <button class="btn btn-primary " id="other-step-button" title="'+result[i]['permissionBy']+'"'+stpbutton+'>Steps Taken</button> <button class="btn btn-primary" id="other-heartrate-button" title="'+result[i]['permissionBy']+'"'+hrtbutton+'>Heart-rate</button> <button class="btn btn-primary" id="other-distance-button" title="'+result[i]['permissionBy']+'"'+disbutton+'>Distance</button>');
				}
				/*for (i = 0; i < result.length; i++)
				{
						console.log(result[i]['permissionBy']);
